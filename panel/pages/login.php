...
if (mysqli_query($sqlconn, "select * from cbt_zona LIMIT 1")==TRUE){
	...
	$h2="DIANGLOBAL-TECHEDUCATION PARTNER";
	$h3="DIANGLOBAL-TECH Login";	
	$hakakses="0";
	$loginpanel="0";
	}
}else{
		...
		<form id="loginform"  name="loginform" onSubmit="return validateForm();" action="ceklogin.php" method="post">
			<div style="text-align:right; width:75%"><!-- Varable 1 userz -->
				...
				<!-- Varable 3 login -->
					...
					<!-- <?php if ($hakakses==0) {?>
					<input type="radio" hidden="" id="radio_siswa" name="loginz" value="siswa" /> <label for="radio_siswa">Siswa</label>
					<?php }?> -->
					<input type="radio" hidden="" id="radio_guru" name="loginz" value="guru" /> <label for="radio_guru">Guru</label>
				</div>
