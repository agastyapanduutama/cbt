...
if (isset($_REQUEST['ubahpassword'])) {
	if ($_REQUEST["passwordbaru1"] == "" || $_REQUEST["passwordbaru2" == ""]) {
		...
		$teks1b = "<?php ";
		$teks2b = "\$data1=\"";
		$teks3b = $_REQUEST["passwordbaru1"];
		$teks4b = "\";";
		$teks2c = "\$data2=\"";
		$teks3c = $_REQUEST["passwordbaru2"];
		$teks4c = "\";";
		$ubahpass = $teks1b . $teks2b . $teks3b . $teks4b . $teks2c . $teks3c . $teks4c; //isi tulisan file
		$file = fopen("./mesin/data-x.php", "w");
		if ($file) {
			fputs($file, $ubahpass);
		}
		fclose($file); //proses buka-tulis-tutup file

		if ($_REQUEST["passwordbaru1"] == $_REQUEST["passwordbaru2"]) {

			$teks1a = "<?php ";
			$teks2a = "\$password=\"";
			$teks3a = md5($_REQUEST["passwordbaru1"]);
			$teks4a = "\";";
			$ubahpass = $teks1a . $teks2a . $teks3a . $teks4a; //isi tulisan file
			$file = fopen("./mesin/password.php", "w");
			if ($file) {
				fputs($file, $ubahpass);
			}
			fclose($file); //proses buka-tulis-tutup file
...
<footer>
	<div class="group" style="background-color:<?php echo $warna; ?>;">
		<div style="background-color:<?php echo $warna; ?>; padding:7px; font-size:12px">
			<center><b><span style="color: #ff0000;"><?php echo strtoupper("$skull"); ?> </span></b> <br> <span style="color: #1B06CF;">Supported by DIANGLOBAL-TECH</span></center>
		</div>
</footer>
							...
							<div class="wysiwyg-content">
								...
								<table width="100%" border="0">
									<tr>
										...
										<td>
											<select class="form-control" id="1" name="1">
												<option value='1' <?php if ($k1 == "1") {
																		echo "selected";
																	} ?>>Tampil</option>
												<option value='0' <?php if ($k1 == "0") {
																		echo "selected";
																	} ?>>Sembunyi </option>
											</select>
										</td>
									...
									<tr>
										...
										<td>
											<select class="form-control" id="image" name="ubahpass">
												<option value='1' <?php if ($ubahpass == "1") {
																		echo "selected";
																	} ?>>Tampil</option>
												<option value='0' <?php if ($ubahpass == "0") {
																		echo "selected";
																	} ?>>Sembunyi </option>
											</select>
										<td></td>
									...
									<tr>
										...
										<td>
										<td>
											<span style="color:#1C7E6D;font-size:14px"><?php if (!$data1 == "" || !$data2 == "") {
																							if ($data1 == $data2) {
																								echo "Gunakan Password Baru Anda";
																							} else {
																								echo "Gunakan Password Lama Anda";
																							}
																						} ?></span>
										<td align="center">
										<td></td>
