...
// if (isset($_COOKIE['PESERTA'])) {
//     unset($_COOKIE['PESERTA']); 
//     setcookie('PESERTA', null, -1, '/'); 
//     return true;
// } else {
//     return false;
// }
...
$tgl2 = date("Y-m-d");
		...
		SELECT * FROM `cbt_siswa_ujian` s LEFT JOIN cbt_jawaban j ON j.XUserJawab = s.XNomerUjian 
		AND j.XTokenUjian = s.XTokenUjian LEFT JOIN cbt_siswa s1 ON s1.XNomerUjian = s.XNomerUjian 
		WHERE s.XNomerUjian = '$user' AND s.XStatusUjian = '1'
		");
		  $namapeserta = $user;
	...
	$pengunana = mysqli_query($sqlconn, "SELECT * from cbt_peserta where nrp_peserta = '$user' ");
	$userna = mysqli_fetch_array($pengunana);
	$namana = $userna['nama_peserta'];

	// var_dump($namana);
	

$xjumbenarz = mysqli_query($sqlconn, "
		  SELECT count(XNilai) as benar FROM cbt_jawaban WHERE XUserJawab = '$user' and XJenisSoal = '1' 
		  and XKodeSoal = '$xkodesoal' and XTokenUjian = '$xtokenujian' and XNilai = '1'
		  ");
		  ...
		  $sqlmasuk = mysqli_query($sqlconn, "INSERT into cbt_nilai ( 
		  	XKodeUjian,XTokenUjian,XTgl,XJumSoal,XBenar,XSalah,XNilai,XKodeMapel,XKodeKelas,XKodeSoal,XNomerUjian,XNamaPeserta,XNIK,XSemester,XSetId,XPersenPil,XPersenEsai,XTotalNilai,XPilGanda,XEsai,XNamaKelas) 
		  values 
		  ('$xkodeujian','$xtokenujian','$tgl2','$xjumsoal','$xjumbenar','$xjumsalah','$nilaix','$xkodemapel','$xkodekelas','$xkodesoal','$user','$namana','$xnik','$xsemester',
                            ...
                            <div class="wysiwyg-content" style="text-align: center;">
                                <p>	Terimakasih telah berpartisipasi dalam tes
									...
									<br>	<font size="2" color="blue">
										<?php	
											echo " Total Soal : ".$kkm."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Benar : ".$xjumbenar."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Salah: ".$xjumsalah."</Font>"; 
