...
<div style="padding-bottom:20px; font-size:14px; color:#0066CC" >Soal Mudah</div>
...
<?php include "config/server.php";
	...
	$kodekelasna = $_SESSION['kodena'];
	$awd = $_SESSION['kodena'];
	$sqluser = mysqli_query($sqlconn, "SELECT * FROM  `cbt_peserta` WHERE nrp_peserta = '$user'");
	$su = mysqli_fetch_array($sqluser);
	$xkelz = $su['kode_kelas'];
	$xjurz = $su['XKodeJurusan'];  
	$sqlgabung = mysqli_query($sqlconn, "
		SELECT * FROM  `cbt_peserta` s
		LEFT JOIN cbt_ujian u ON (u.XKodeKelas = '$kodekelasna') 
		WHERE nrp_peserta = '$user' 
		AND (u.XKodeJurusan = '$xjurz' or u.XKodeJurusan = 'ALL') 
		AND (u.XKodeKelas = '$xkelz' or u.XKodeKelas = 'ALL') 
	...
	$bajingan = "
		...
		WHERE u.XKodeSoal='$xkodesoal' 
		AND u.XStatusUjian = '1'
		";
	$sqluser = mysqli_query($sqlconn, $bajingan);
	$so = mysqli_fetch_array($sqluser);
	$sopil = $so['XJumPilihan'];
	$listen =$so['XListening'];
	$hadeh = "
		SELECT j.Urut AS Urut, j.XJawaban AS XJawaban, j.XJawabanEsai AS XJawabanEsai , c.XNomerSoal AS XNomerSoal, j.XRagu AS XRagu, c.XJenisSoal AS XJenisSoal, C.XKategori AS Kategorina
		FROM  `cbt_soal` c 
		LEFT JOIN cbt_jawaban j ON ( j.XNomerSoal = c.XNomerSoal AND j.XKodeSoal = c.XKodeSoal ) 
		WHERE c.XKodeSoal = '$kodekelasna' 
		AND j.XUserJawab = '$user' 
		AND j.XTokenUjian = '$token' 
		AND j.XJenisSoal = '1' 
		AND c.XKategori = '1'
		ORDER BY j.Urut";

	$sql = mysqli_query($sqlconn, $hadeh);
		while($s = mysqli_fetch_array($sql)){		
			...
			if($s['XRagu']=='1'){
				$cssb = "#eaca08"; $csst = "#000";
				$noti = "noti-ragu";
				$border = "#eaca08";
				$iki = 'R';
			}elseif($s['Urut']==$_REQUEST['pic'] ){
				$cssb = "#336898"; $csst = "#fff";
				$noti = "noti-ragu";
				$border = "#336898";
				$iki = 'S';
			}elseif(!$jwbsoal==''){
				$cssb = "#313132"; $csst = "#fff";
				$noti = "noti-ragu";
				$border = "#313132";
				$iki = 'S';
			}else {	if(!$s['XJawaban']==''){
...
<div style="padding-bottom:20px; font-size:14px; color:#0066CC" >Soal Sedang</div>
...
<div style="padding-bottom:20px; font-size:14px; color:#0066CC" >Soal Sulit</div>
<div id="container3" style="text-align:center; height:300px;">
	<?php include "config/server.php";
	$xkodemapel = "GAL1";
	$user = "$_COOKIE[PESERTA]";
	$token = $_SESSION['token'];
...
<div id="container4" style="text-align:center; height:300px;">
<?php include "config/server.php";
	$xkodemapel = "GAL1";
	...
	$sqlgabung = mysqli_query($sqlconn, "
		...
		WHERE nrp_peserta = '$user' AND (u.XKodeJurusan = '$xjurz' or u.XKodeJurusan = 'ALL') 
		AND (u.XKodeKelas = '$xkelz' or u.XKodeKelas = 'ALL') AND u.XStatusUjian = '1'

		-- SELECT * FROM  `cbt_peserta` s 
		-- LEFT JOIN cbt_ujian u ON (s.kode_kelas = u.XKodeKelas or u.XKodeKelas = 'ALL') 
		-- WHERE nrp_peserta = '$user' AND (u.XKodeJurusan = '$xjurz' or u.XKodeJurusan = 'ALL') 
		-- AND (u.XKodeKelas = '$xkelz' or u.XKodeKelas = 'ALL') AND u.XStatusUjian = '1'
		");
	$s0 = mysqli_fetch_array($sqlgabung);
	...
	$xtokenujian = $_SESSION['token'];  
 	$anying= "
		...
		AND u.XStatusUjian = '1'
		";
	$sqluser = mysqli_query($sqlconn, $anying);
	$so = mysqli_fetch_array($sqluser);
	$sopil = $so['XJumPilihan'];
	...
	$sql = mysqli_query($sqlconn, "
		...
		AND j.XKodeSoal = c.XKodeSoal ) 
		WHERE c.XKodeSoal = '$xkodesoal' 
		AND j.XUserJawab = '$user' 
		AND j.XTokenUjian = '$xtokenujian'
		AND j.XJenisSoal = '2' 
		ORDER BY j.Urut
		");
	while($s = mysqli_fetch_array($sql)){
		...
		$jwbsoal = $s['XJawabanEsai'];
		$urutansoal = $s['Urut'];
		if($urutansoal==""){$uresaiutansoal=1;}else{$urutansoal=$urutansoal;}
...
";
$sqlj = mysqli_query($sqlconn, $ieumeren);
...
<?php 
$soalna = query_get_row("SELECT * FROM cbt_soal");
...
<?php
 ...
 $cekesai = mysqli_query($sqlconn, "
 	SELECT * FROM cbt_jawaban where Urut='$current_picture' and XKodeSoal ='$xkodesoal' and XTokenUjian = '$xtokenujian' and XUserJawab = '$user' 
 	"); 
